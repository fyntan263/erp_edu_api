package com.innoverse.erp_edu_api.provisioning.api;
...
// Add these endpoints to your ProvisioningController
    ...
    @PostMapping("/maintenance/cleanup/orphaned-schemas")
    ...
    public ResponseEntity<ApiResponse> deleteAllOrphanedSchemas() {
        log.info("Deleting all orphaned schemas");

        try {
            // Get all orphaned schemas first
            List<String> orphanedSchemas = orchestrationService.findOrphanedSchemas();
            ...
            if (orphanedSchemas.isEmpty()) {
                return ResponseEntity.ok()
                        .body(new ApiResponse(
                                "SUCCESS",
                                "No orphaned schemas found to delete"
                        ));
            }

            log.info("Found {} orphaned schemas to delete: {}", orphanedSchemas.size(), orphanedSchemas);

            // Delete all orphaned schemas
            int deletedCount = orchestrationService.deleteAllOrphanedSchemas();

            return ResponseEntity.ok()
                    .body(new ApiResponse(
                            "SUCCESS",
                            "Deleted " + deletedCount + " orphaned schemas: " + orphanedSchemas
                    ));
        } catch (Exception e) {
            throw new ProvisioningException(
                    "PROVISION",
                    "Failed to delete orphaned schemas",
                    "Unexpected error during orphaned schema deletion: " + e.getMessage(),
                    e
            );
        }
    }
